...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/19 20:44:08 by abiju-du          #+#    #+#             */
/*   Updated: 2022/01/19 20:44:11 by abiju-du         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin(const char *s1, const char *s2, int need2free)
{
	char		*str_join;
	size_t		s1_len;
	size_t		s2_len;
...
int	get_next_line(int fd, char **line)
{
	...
	while (n_search(str[fd]) == -1 && bytes_read != 0)
	{
		...
		if (read_error(bytes_read, line) == -1)
			return (-1);
		buf[bytes_read] = 0;
		str[fd] = ft_strjoin(str[fd], buf, 1);
	}
